...
import { getChargesData } from '../../helpers/apiCalls'
...
const BarGraphCard = () => {
  ...
  useEffect(() => {
    const getDataCallback = async () => {
      const res = await getChargesData()
      const temp_data = {
        keys: Object.keys(res),
        values: Object.values(res),
      }
      dispatchFilterContextValue({ type: 'setLabels', payload: temp_data.keys })
      setData(temp_data)
    }
  ...
  return (
    <div className="flex w-4/5 flex-col col-span-full sm:col-span-6 bg-white shadow-lg rounded-sm border border-slate-200">
      <header className="px-5 py-4 border-b border-slate-100">
        <h2 className="font-semibold text-slate-800">Remaining Charges</h2>
      </header>
